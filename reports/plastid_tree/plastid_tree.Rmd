...
knitr::opts_chunk$set(echo = FALSE, message = FALSE, warning = FALSE, results = "hide", cache = FALSE, fig.retina = 3, fig.align = "center")
library(phytools)
```

```{r load-data}
# Load Catalog of Life plants taxonomy
loadd(col_plants, cache = ftol_cache)

# Load time tree inferred from plastid data
phy <- readd(plastid_tree_dated, cache = ftol_cache)

# Load Pteridopyte Phylogeny Group I taxonomy
ppgi <- readd(ppgi_taxonomy, cache = ftol_cache)
...
clade_data <-
  ...
  mutate(taxon = paste3(genus, specificEpithet, infraspecificEpithet, sep = "_")) %>%
...
```{r tree-numbers, echo=FALSE, warning=FALSE, message=FALSE, error = FALSE}
# Check the total number of ingroup samples
n_ingroup <- tips %>%
  filter(!genus %in% outgroup_genera) %>%
  pull(tip) %>%
  n_distinct()
...
overall_coverage <-
n_ingroup %>% magrittr::divide_by(n_accepted_ferns) %>% scales::percent()

cov_high <-
clade_data %>% arrange(desc(coverage)) %>% slice(1) %>% pull(coverage) %>% scales::percent()

cov_high_name <-
clade_data %>% arrange(desc(coverage)) %>% slice(1) %>% pull(clade) %>% as.character()

cov_low <-
clade_data %>% arrange(coverage) %>% slice(1) %>% pull(coverage) %>% scales::percent()

cov_low_name <-
clade_data %>% arrange(coverage) %>% slice(1) %>% pull(clade) %>% as.character()

cov_pteridineae <-
clade_data %>% filter(clade == "Pteridineae") %>% pull(coverage) %>% scales::percent()
```

The current FTOL includes `r scales::number(n_ingroup, big.mark = ",")` tips (**Fig. 1**). 

Overall coverage (percent of species in the tree out of all accepted species) is `r overall_coverage` (**Fig. 2**). Coverage by major clade (order or suborder; taxonomy follows [Pteridophyte Phylogeny Group 2016](https://doi.org/10.1111/jse.12229)) varies from `r cov_low` (`r cov_low_name`) to `r cov_high` (`r cov_high_name`). Larger clades tend to have lower coverage, with the exception of Pteridineae (`r cov_pteridineae` of 1,532 accepted names).
...
# # Dark version (for some reason, can't plot both phylo and arc.cladelabels on black background)
# pdf(file = "phy_dark_labels.pdf", height = 10, width = 10)
# # par(fg="white",bg="grey10")
# plot.new()
# # phytools::plotTree(phy, type="fan", ftype="off", lwd = 0.8)
...
# 
# # Dark version
# pdf(file = "phy_dark.pdf", height = 10, width = 10)
# par(fg="white",bg="grey10")
# phytools::plotTree(phy, type="fan", ftype="off", lwd = 0.8, color="grey90")
# dev.off()
```

**Figure 1: Fern tree of life (FTOL) based on plastid data.** Colors correspond to major clade (order or suborder, see **Fig. 2** for labels). Shading on tree indicates geologic era (Cenozoic not labeled).
...
**Figure 2: Sampling completeness.** Barplot shows coverage (percent of species in tree out of all accepted species); numbers next to bars indicate the total number of accepted species in each major clade.
