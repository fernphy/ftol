...
- restez_sql_db.tar.gz: Compressed file archive including database of fern DNA sequences from GenBank release 249.
...
sequences downloaded from the NCBI GenBank FTP site (release 249,
https://ftp.ncbi.nlm.nih.gov/genbank/) formatted as a MonetDB database
(https://www.monetdb.org/), created with restez R package
v1.1.0 (https://github.com/ropensci/restez). The sequences can also be accessed
with restez. To do so, the contents of the archive (folders "bat" and
"sql_logs") need to be extracted to a folder "restez/sql_db". Then, run the
restez function restez_path_set() with the folder containing "restez/sql_db" as
input.
