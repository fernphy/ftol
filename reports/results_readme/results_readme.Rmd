...
- `ftol_plastid_accs.csv`: Table of GenBank accessions used for phylogenetic analysis. Columns include `accession` (GenBank accession number), `species` (species name after taxonomic standardization), `gene` (gene name; NA for accessions from full plastomes), and `plastome_title` (GenBank accession for full plastomes; NA for Sanger sequences). `r nrow(plastid_acc_data) %>% scales::number(big.mark = ",")` rows x `r ncol(plastid_acc_data)` columns.

- `ftol_plastid_list.RDS`: List of plastid sequences used for phylogenetic analysis. Each item of the list is an alignment (list of class 'DNAbin') corresponding to a single gene, with sequences named by species. All sequences aligned with MAFFT and trimmed with TrimAl. R data object saved in the RDS file format. 

- `ftol_plastid_concat.fasta`: Concatenated alignment of plastid genes used for phylogenetic analysis. `r n_concat_species` species x `r n_concat_bases` base pairs, including `r length(plastid_genes_aligned_trimmed_renamed)` genes. `r p_concat_missing` missing data. FASTA format.

- `ftol_plastid.tre`: Fern tree of life (FTOL) based on plastid genes, not dated. `r n_tip` species (one tip per species), including `r n_ingroup_tips` ingroup and `r n_outgroup_tips` outgroup taxa. Branch lengths in units of expected change per site. Newick format.

- `ftol_plastid_dated.tre`: Fern tree of life (FTOL) based on plastid genes, with divergence times estimated using fossil calibration points. Taxon sampling same as `ftol_plastid.tre`. Branch lengths in millions of years. Newick format.
